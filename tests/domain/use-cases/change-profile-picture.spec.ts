...
describe('ChangeProfilePicture', () => {
  ...
  beforeAll(() => {
    ...
    userProfileRepo.load.mockResolvedValue({ name: 'Gustavo Camargo Eklund' })
  })
  ...
  it('should call SaveUserPicture with correct input when file is undefined', async () => {
    ...
    expect(userProfileRepo.savePicture).toHaveBeenCalledWith({ pictureUrl: undefined, initials: 'GE' })
    expect(userProfileRepo.savePicture).toHaveBeenCalledTimes(1)
  })

  it('should call SaveUserPicture with correct input when file is undefined', async () => {
    userProfileRepo.load.mockResolvedValueOnce({ name: 'gustavo Camargo eklund' })
